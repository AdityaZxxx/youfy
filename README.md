...
- **🖼️ Meta-Tagged:** Automatically embeds album art (thumbnails) and metadata into your MP3/FLAC files.
...
- **🛡️ Rate Limit Safe:** configurable Spotify credentials to avoid API limits.

## Installation

1. Clone this repository:
   ```bash
   git clone <your-repo-url> spotube
   cd spotube
   ```
...
3. (Optional) Create a global symlink:
   ```bash
   sudo ln -s $(pwd)/run.sh /usr/local/bin/spotube
   ```
   *Now you can run `spotube` from anywhere!*
...
3. Run Spotube with a Spotify link for the first time.
4. Paste your **Client ID** and **Client Secret** when prompted.

Configuration is stored securely in `~/.config/spotube/config.json`.
